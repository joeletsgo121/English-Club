...
document.getElementById("loginBtn").addEventListener("click", () => {
  openModal("Login", `
    <form id="loginForm">
      ...
      <p id="loginMessage" style="margin-top:12px;font-size:13px;"></p>
    </form>
  `);
...
document.getElementById("registerBtn").addEventListener("click", () => {
  openModal("Create your account", `
    <form id="registerForm">
      <div class="form-group">
        ...
        <input id="registerName" required placeholder="Joe">
      </div>
      <div class="form-group">
        <label>Email</label>
        <input id="registerEmail" type="email" required placeholder="you@example.com">
      </div>
      <div class="form-group">
        <label>Password</label>
        <input id="registerPassword" type="password" minlength="6" required placeholder="Create a password">
      </div>
      <button class="modal-submit" type="submit">Create Account</button>
      <p id="registerMessage" style="margin-top:12px;font-size:13px;"></p>
    </form>
  `);
  ...
  document.getElementById("registerForm").addEventListener("submit", async event => {
    ...
    const { data, error } = await supabaseClient.auth.signUp({
      ...
      options: {
        data: { full_name: name },
        emailRedirectTo: "https://joeletsgo121.github.io/English-Club/"
      }
    });
    ...
    if (data.session) {
      ...
      message.textContent = "Account created! Please check your email to confirm your account.";
    }
  });
});
